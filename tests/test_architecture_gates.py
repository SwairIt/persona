...
# Reviewed +1 (2026-08-26): ЛЮДИ И РОСТ — один роут, GET /root/people
# (app/web/routes/people_owner.py).
#
# Что владелец попросил: видеть, КТО зарегистрировался (адреса, даты) и КАК
# идёт прирост — сегодня против вчера, неделя против недели, месяц против
# месяца, — и при этом НЕ видеть содержимого чужих чатов и памяти.
#
# Почему это отдельная страница, а не блок внутри /root/analytics (то есть
# ноль новых роутов). Две причины, обе структурные:
#
# 1. Источник данных другой. Дашборд аналитики честно рисует рамку «события
#    с такого-то числа» — раньше даты включения счётчика у него данных нет.
#    Регистрации считаются по users.created_at и верны за ВСЮ историю
#    инстанса. Помесячный график за год внутри рамки «данные с 25 августа»
#    читался бы как обрезанный, хотя он полон.
# 2. Правило приватности другое, и оно должно быть видно целиком. На этой
#    странице чужой текст не показывается НИКОГДА: в шаблон приходят
#    dataclass'ы Person со slots=True (id, адрес, роль, статус, даты,
#    счётчики), а модуль app/analytics/people.py не упоминает chat_message /
#    dm_message / user_memory даже по имени — это проверяется тестом по
#    исходнику. Держать такое правило куском большой страницы, где рядом
#    крутятся пути, клики и метки элементов, значит потерять его при первой
#    же правке соседнего блока.
#
# Мутаций аккаунта эта страница НЕ добавила: «заморозить», «вернуть» и
# «удалить» отправляются в уже существующий POST /root/users/{uid}/{op} с
# полем next. Второй набор ручек означал бы два места, где решается «можно ли
# снести этот аккаунт», и расхождение гардов. Там же delete переведён с
# голого DELETE FROM users на каскад app/auth/account_delete.py — это правка
# существующего роута, а не новый.
#
# Значение ПЕРЕМЕРЕНО на живом create_app() (1164), а не получено сложением.
# Reviewed +1 (2026-08-29): СКРИНШОТЫ СТАТЬИ — один роут, GET /habr-img/{name}
# (app/web/routes/habr_img.py).
#
# Что понадобилось: статья публикуется на Хабре с другой машины, а редактор
# Хабра в markdown-режиме принимает только готовый адрес картинки внутри
# `![подпись](url)`. Значит скриншоты должен кто-то отдавать по HTTP.
#
# Почему это не свелось к нулю новых роутов — рассмотрены оба очевидных
# варианта, и оба хуже:
#
# 1. Скопировать картинки в app/web/static/ и отдать существующим маунтом.
#    Тогда файлы к статье лежат в двух местах: в docs/habr-screenshots/ рядом
#    с текстом, который на них ссылается, и в статике приложения. Первая же
#    пересъёмка разведёт копии, и статья будет показывать старое. Картинки —
#    часть документа, а не ассет интерфейса.
# 2. Примонтировать docs/ как StaticFiles. Это опубликовало бы ВСЮ папку
#    документации разом — включая внутренние заметки и планы. Ради одиннадцати
#    jpg открывать наружу целое дерево нельзя.
#
# Сам роут уже в консолидированной форме: один параметризованный обработчик
# на все файлы, а не по литеральному роуту на картинку. Имя проверяется
# строгим шаблоном плюс проверкой, что путь не вышел за пределы папки.
#
# Значение ПЕРЕМЕРЕНО на живом create_app() (1165).
REGISTERED_ROUTE_BUDGET = 1_165  # +6 (2026-08-24): AI-everywhere wave 2 (insights, suggest, hour summary, ai-calendar parse+create); +15 (2026-08-24): social layer; +6 (2026-08-24): llm sharing grants; +8 (2026-08-24): DM AI replies + social notifications; +3 (2026-08-25): owner analytics; +5 (2026-08-25): поддержка (форма + ящик + бейдж); +6 (2026-08-25): блог — поиск, ленты, категории, теги, sitemap-индекс; +1 (2026-08-26): люди и рост (/root/people); +1 (2026-08-29): скриншоты статьи (/habr-img)
