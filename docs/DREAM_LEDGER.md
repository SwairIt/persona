...
6. REM reflection, `dream_report`, processed-message cursor, terminal
   `dream_run` state, and completion audit commit in the same transaction.
...
Graph projection and post-apply embeddings should consume applied revisions in
a separate idempotent outbox; they are not performed inside the memory write
transaction.
