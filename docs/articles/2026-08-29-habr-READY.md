...
**Persona** — [persona.getdoday.ru](https://persona.getdoday.ru). Личный ИИ-ассистент с приватной памятью: он помнит ваши разговоры, строит по ним граф связей и работает на вашей модели. Можно подключить свой ключ к облачному провайдеру, можно локальную Ollama — данные не уезжают к чужому дяде. Стек: FastAPI, SQLite, Jinja, htmx, Alpine, Tailwind. Без React — я его просто не знаю настолько, чтобы на нём было быстрее, чем без него.

**Про код сразу, чтобы не было недомолвок.** Doday лежит на GitHub под MIT, а репозиторий Persona пока закрыт. Не из желания что-то спрятать: там мультипользовательский режим и шифрование данных участников, и я не готов открывать это до нормального аудита — слишком легко выложить вместе с кодом чью-нибудь дырку. Поэтому все куски кода в статье я привожу прямо в тексте, целиком и как есть, вместе с комментариями, которые реально лежат в файлах. Если чего-то не хватает для разбора — спрашивайте в комментариях, выложу.
