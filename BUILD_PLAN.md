...
- [x] D9+. Доп. темы из синтеза ресёрча (D9-D15 закрыты, блог 15/15).

## ФАЗА E — Ревью, тесты, дебаг, финал
- [x] E1. Прогон всего pytest + golden-eval (память не просела).
- [x] E2. Ревью: новые роуты под auth-gate/owner, jinja-компиляция всех новых шаблонов,
  миграции идемпотентны 2×, нет битых ссылок навигации день↔граф↔память.
- [x] E3. Дебаг найденного.
- [x] E4. Финальный отчёт: что сделано/не сделано, +коммиты, итог LOC.
...
- D12+D13 ✅ (v2.20.50): golosovoy-dostup-k-pamyati.md (Гайды — голосовой цикл STT→ход→TTS, hands-free, barge-in,
  два транспорта браузер/агент, движки на устройстве=local-first; честная оговорка про качество) и
  nosimye-kulony-vs-lokalnaya-pamyat.md (Сравнения — Limitless Pendant/Bee/Plaud/Friend: облако, согласие
  окружающих, судьба стартапа Rewind→Meta; запись своего экрана/звука контролируемее; где носимые объективно
  сильнее; таблица). Обе 200 анонимно, list_posts=26. Блог 13/15. Осталось D14 (Ollama офлайн), D15 (vs MS Recall).
- D14+D15 ✅ (v2.20.51): podklyuchaem-ollama-k-persona.md (Гайды — НЕ дубль существующего ollama-lokalnyy-zapusk:
  именно подключение Ollama к Persona как локального провайдера, чат-модель + embed nomic-embed-text для
  семантического recall, air-gapped, опциональность векторного режима, честно про скорость/качество, перелинковка)
  и persona-vs-microsoft-recall.md (Сравнения — 5 различий: открытость/NPU/экран+звук/слой ассистента/экосистема,
  история безопасности Recall, таблица, где Recall объективно удобнее, кому что). Обе 200 анонимно, list_posts=28.
  ⭐ ФАЗА D (блог) ПОЛНОСТЬЮ ЗАКРЫТА: 15/15 новых статей + 13 существовавших = 28 постов. → Переход в Фазу E.
- ⭐ ФАЗА E ✅ (v2.20.52): E1 — полный pytest: 535 passed, 14 skipped; найдено 2 фейла (test_home_renders,
  test_help_renders) — оказались ПРОТУХШИМИ ТЕСТАМИ (дрейф фикстур после прошлых перестановок маршрутов:
  «/» переехал в landing.py, «/help» → help_walkthrough.py 2026-06-07; help.py теперь отдаёт /help/shortcuts),
  НЕ регрессии ночной работы. Подтверждено на живом сервере: /, /help, /help/shortcuts = 200. E3-дебаг: оба теста
  починены минимально и корректно (test_web_routes монтирует landing.router=владелец «/»; test_help бьёт
  /help/shortcuts) → 14/14 зелёные, маскировки нет (продукт доказанно здоров). E2-ревью: все 15 /blog/{slug} +
  11 публичных роутов (/features,/compare/{rewind,recall},/pricing,/security,/privacy-policy,/terms,/roadmap,
  /changelog,/blog,/landing) = 200 анонимно; мои 7 новых шаблонов компилируются; auth-gate держит кабинет
  (/day,/analytics,/memory,/settings → 303 анониму); миграции идемпотентны (повторный прогон → statement_skipped
  без ошибок). E4 — финальный отчёт пользователю выдан. ⭐ ВЕСЬ BUILD_PLAN (A→E) ЗАКРЫТ. СТОП.
