...
def _frame_path(shot_id: int, style: str) -> Path:
    ...
    per ``(shot_id, style)``. The directory is created lazily by the
    renderer.
    """
    tmp_dir = Path(tempfile.gettempdir()) / "persona-screenshot-frame"
    return tmp_dir / f"persona-frame-{shot_id}-{style}.png"
...
async def screenshot_frame_png(
    ...
    out_path = _frame_path(screenshot_id, typed_style)
    result = await build_framed_png(screenshot_id, out_path, style=typed_style)
