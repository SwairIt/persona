...
_PUBLIC_PREFIXES: tuple[str, ...] = (
    ...
    "/changelog",
    "/sitemap.xml",
    # Дочерние карты sitemap-индекса (/sitemap-pages.xml, -posts, -categories,
    # -tags, -public). Индекс в /sitemap.xml ссылается на них, и если бы гейт
    # отдавал по этим адресам 303 на /landing, поисковик получил бы индекс,
    # ведущий в пустоту — хуже, чем отсутствие карты. Открывать тут нечего:
    # единственный роут на этом префиксе (app/web/routes/sitemap.py) отдаёт
    # только те URL, которые и так публичны, и на неизвестную секцию отвечает
    # 404, а не пустым urlset.
    "/sitemap-",
    "/robots.txt",
    # Форма обратной связи (GET + POST /support). Открыта анониму НАМЕРЕННО:
    # человек, который не может войти, — как раз тот, кому нужнее всего
    # написать владельцу. Ящик владельца лежит НЕ здесь, а на
    # /settings/support: совпадение тут префиксное, и любой путь под /support
    # стал бы публичным вместе с ним.
    "/support",
    "/auth/",
    "/help",
    "/static/",
    ...
    # Публичный НАМЕРЕННО: события с лендинга и других открытых страниц шлёт
    # анонимный посетитель, и без этой строки верх воронки («что нажимают до
    # регистрации») не измерялся бы вовсе — гейт отдавал бы 401.
    # Открывать тут нечего: ручка ничего не читает и не отдаёт, а только
    # принимает подпись нажатого элемента, ПЕРЕЗАПИСЫВАЕТ путь по своей
    # таблице роутов и роль берёт из сессии, а не из тела. Событие анонима БЕЗ
    # куки согласия она отбрасывает сама (app/web/routes/analytics_owner.py).
    "/api/track",
    "/api/sync/",
...
_MEMBER_PREFIXES: tuple[str, ...] = (
    ...
    # «Мои данные»: выгрузка своих данных и удаление своего аккаунта (152-ФЗ).
    # Роуты в app/web/routes/my_data.py НЕ принимают id пользователя ни в
    # каком виде — личность берётся только из сессии, поэтому чужие данные тут
    # недостижимы by construction. ВЛАДЕЛЬЧЕСКАЯ /settings/privacy сюда
    # НАМЕРЕННО не добавлена: её /settings/privacy/snapshot делает VACUUM INTO
    # по всей базе инстанса (чужие чаты + kv-секреты), а остальные её тумблеры
    # инстанс-глобальны. Префикс «/settings/my-data» не является префиксом
    # «/settings/privacy» и наоборот — _is_member_path сравнивает по границе
    # сегмента, так что открыть одно через другое нельзя.
    "/settings/my-data",
)
