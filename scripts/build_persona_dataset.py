...
async def _main() -> None:
    ...
    if args.include_history and args.user_id:
        # «Вторая копия» — копия ВЛАДЕЛЬЦА. ``--user-id`` чужого аккаунта собрал
        # бы файл дообучения из личной переписки постороннего человека: на
        # инстансе открыта публичная регистрация, и такие аккаунты там есть.
        # Доступ к файлу базы у оператора и так есть (это честно написано в
        # политике) — но продукт не обязан делать выгрузку чужих чатов удобной
        # однострочной командой. Отказ без флага-обхода: обход тут и был бы
        # той самой удобной командой.
        from app.auth.owner import is_owner  # noqa: PLC0415

        if not await is_owner(args.user_id):
            print(
                f"Отказ: user-id={args.user_id} — не владелец инстанса. "
                "История чатов участника в датасет дообучения не собирается.",
                file=sys.stderr,
            )
            raise SystemExit(2)
